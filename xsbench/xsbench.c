...
#define P_FACTOR 12500000000UL  // 100GB target
#define G_FACTOR 210000
#define LOOKUPS 6500000000UL
...
double run_lookups(Simulation *sim, uint64_t n_lookups) {
    double result = 0.0;
    uint64_t n_threads = omp_get_max_threads();
...
int main(int argc, char *argv[]) {
    ...
    printf("XSBench Scaled - MS Configuration\n");
    printf("Threads: %d\n", omp_get_max_threads());
    printf("p_factor: %lu, g_factor: %lu\n", p_factor, g_factor);
    printf("Lookups: %lu\n\n", lookups);
